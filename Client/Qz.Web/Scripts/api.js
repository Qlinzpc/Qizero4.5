...
(function (window) {
    ...
    u.addCls = function (el, cls) {
        ...
        if (typeof el === 'object' && u.isNodeList(el)) {
            i = el.length;
            while (i--) {
                if (u.hasCls(el[i], cls)) { u.removeCls(el[i], cls); }
            ...
            if (len > 1) {
                for (i = 0; i < len; i++) {
                    if (u.hasCls(el, acls[i])) { u.removeCls(el, acls[i]); }
                }
            } else {
                if (u.hasCls(el, cls)) { u.removeCls(el, cls); }
            }
...
(function (o) {
    ...
    Select.prototype.init = function () {
        ...
        $api.removeCls(this.box.arr, 'select');
