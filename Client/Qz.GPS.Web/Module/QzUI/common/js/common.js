    ...
    var common = new function () {
        ...
        this.loading = new function () {
            this.show = function (type) {
                ...
                // 遮蔽层
                $api.remove($api.dom('.l-mask[data-ajax]'));
                $api.append($api.dom('body'), '<div class="l-mask animated fadeIn" style="display:block;" data-ajax></div>');
                $api.show(_this);
...
Array.prototype.indexOf = function (val, is) {
    ...
    for (i = 0; i < this.length; i++) {
        if (is == 0 && compare( this[i] , val)) return i;
